        ...
        <div class="layui-card-body">
            <table id="my-table-data" lay-filter="my-table-data">
                <script type="text/html" id="my-table-toolbar">
                    <a class="layui-btn layui-btn-xs" lay-event="child">添加子权限</a>
...
<script>
    ...
    layui.config({
        ...
        table.render({
            ...
            cols: [[
                {field: '_name', title: '名称'},
                {field: 'name', title: '权限'},
                {field: 'is_menu', title: '是否是菜单'
                    ,templet: function(d){
                        return d.is_menu == 1 ? '<font style="color:red">是</font>' : '否'
                    }
                },
        ...
        table.on('tool(my-table-data)', function (obj) {
            ...
            if (obj.event === 'close') {
                ...
                layer.open({type: 2, title: '添加子权限', shadeClose: true, shade: 0.8, area: ['50%', '50%'], maxmin: true, content: 'child?pid=' + data['id']});
                return false;
            }
        });
    ...
    jQuery.change_status = function (options) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                } else layer.msg(data.message, {icon: 5});
            },
            error: function () {
